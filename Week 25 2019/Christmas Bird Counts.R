...
bird_top %>% 
  ...
  ggplot(aes(species, total, fill = highlight_flag)) +
  geom_col() +
  geom_text(aes(label=total), hjust = -0.1, color = 'black') +
  scale_fill_manual(values = c('grey65', 'skyblue4')) +
  labs(title = "Top Ten Species Observed Annually",
       subtitle = "Based on the sum of observations 1921 - 2017") +
  coord_flip()
...
bird_time %>% 
  ...
  geom_line(aes(year, how_many_counted, group = species), size = 0.65, data = bird_filtered, color = 'skyblue4')
